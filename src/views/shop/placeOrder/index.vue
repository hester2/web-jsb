...
<script>
  ...
  import OrderGoodsTable from '@/views/shop/components/OrderGoodsTable'
  ...
  export default {
    ...
    computed: {
      ...mapState('order', ['orderDetails'], 'userInfo'),
      // 货品总金额
      orderTotalAmount: function() {
        const arr = this.orderGoods.map(item => {
          return item.orderAmount * item.perPrice
        })
    ...
    created() {
      ...
      this.orderGoods = this.orderDetails
    ...
    methods: {
      ...
      // 提交订单
      async submitOrder() {
        // goodsName goodsOrderDetailVo materialsType orderAmount shopId userId
        ...
        const data = {
          ...
          materialsType: '',
          orderAmount: this.orderTotalNumber,
          shopId: this.orderGoods[0].shopId,
          type: 2,
